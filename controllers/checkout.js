...
module.exports.getCheckout = async(req, res, next)=>{
    if(req.cookies.cart){
        res.render('checkout/checkout', {path:'/checkout', title:'checkout', session:req.cookies.token?true:false})
    }
    else{
        res.redirect('/shop')
    }
...
try {
    ...
    if(createOrder){
        res.cookie('order', createOrder.id, { expires: new Date(Date.now() + 900000)})
        res.redirect('/checkout/checkout')
    }


}catch (error) {
    res.render('popup.ejs', {message:error, title:'popup', path:'/popup', url:'shop', session:req.cookies.token?true:false})
}
...
module.exports.getSuccess = async(req, res, next)=>{
    res.clearCookie('cart')
    res.clearCookie('order')
    res.render('checkout/success', {path:'/success', title:'success', session:req.cookies.token?true:false})
}